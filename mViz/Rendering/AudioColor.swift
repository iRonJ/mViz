...
func reactiveColor(bands: SIMD3<Float>, offset: Float) -> SIMD3<Float> {
...
#if canImport(UIKit)
  /// Computes particle start and end colors incorporating base hue, beat palette offset, high-frequency sizzle, beat brightness pulse, and silence dimming.
  func reactiveEmitterColors(
    ...
    let act = min(1.0, max(0.0, activity))
    ...
    // Particle brightness dynamically pulses with the beat and dims in quiet/silence!
    // In quiet (act = 0): ~0.18. Active track (act = 1.0): ~0.60. On beat pulse: surges up to 1.0.
    let startBrightness = CGFloat(min(1.0, 0.18 + act * 0.42 + beat * 0.40 + sizzle * 0.18 + bands.x * 0.08))
    ...
    // with its trailing brightness also breathing with the beat and dimming in silence.
    var rawEndHue = (Float(hue) + 0.14 + sizzle * 0.08).truncatingRemainder(dividingBy: 1.0)
    if rawEndHue < 0 { rawEndHue += 1.0 }
    let endBrightness = CGFloat(min(0.85, 0.05 + act * 0.35 + beat * 0.35 + sizzle * 0.15))
