...
final class ParticleField {
  ...
  func recenterStage() {
    // Stage positions are seamlessly maintained in the forward view.
  }

  var emitters: [Entity] = []
  var emitterSlots: [[Entity]] = []
  var activeParticleStyle: ParticleStyle? = nil
  var time: Float = 0
  ...
  private var rhythmTracker = RhythmBopTracker<SIMD3<Float>>()
  ...
  var bopEnvelope: SIMD3<Float> { rhythmTracker.bop }
  ...
  init() {
    ...
    let initialStyle = activeStyle(shapeTime: 0, selectedStyle: .evolving)
    activeParticleStyle = initialStyle

    for index in 0..<12 {
      let entity = Entity()
      entity.name = "Emitter \(index)"
      entity.position = blend.position(index: index, phase: Float(index) / 12 * 2 * .pi, time: 0, bass: 0)
      ...
      for slot in 0..<3 {
        ...
        let child = Entity()
        child.name = "Emitter \(index) Slot \(slot)"
  ...
  func update(dt: Float, model: VisualizerModel) {
    ...
    rhythmTracker.update(signal: signal, dt: dt)
    ...
    let bop = rhythmTracker.bop
    let sustainedBass = pow(envelope.x, 1.15)
    let sustainedMid = pow(envelope.y, 1.10)
    let sustainedTreble = pow(envelope.z, 1.10)
    ...
    let targetStyle = activeStyle(shapeTime: shapeTime, selectedStyle: model.particleStyle)
    let styleChanged = (targetStyle != activeParticleStyle)
    if styleChanged {
      activeParticleStyle = targetStyle
    }

    let targetShape: ParticleEmitterComponent.EmitterShape
    if model.shape == .evolving, let form = activeDynamics.form {
      targetShape = form == .plane ? .plane : .cone
    } else {
      targetShape = currentForm.shape
    }

    let baseStretch: Float = (targetStyle == .sparks ? 2.8 : 1.0)
    let modeStretch: Float = activeDynamics.stretch ?? 1.0
    let styleSizeMultiplier: Float = (targetStyle == .rings || targetStyle == .flakes) ? 1.8 : 1.0
    let styleLifeMultiplier: Double = (targetStyle == .sparks) ? 0.85 : 1.0
    ...
    for index in 0..<12 {
      ...
      let currentPos = blend.position(index: index, phase: phase, time: time, bass: envelope.x)
      entity.position = currentPos

      let rollQuat = simd_quatf(
        angle: time * 0.25 + phase, axis: normalize(SIMD3<Float>(1, 0.5, 0.3)))
      if stageWeight > 0.001 {
        let flatQuat = simd_quatf(angle: 0, axis: [0, 1, 0])
        entity.orientation = simd_slerp(rollQuat, flatQuat, min(1, stageWeight * 1.5))
      } else {
        entity.orientation = rollQuat
      }
      ...
      if stageWeight > 0.001 {
        let side: Float = currentPos.x < 0 ? -1 : 1
        let stageFan = normalize(SIMD3<Float>(side * 0.12, 1.0, -0.08))
        let mixed = blendedDir * (1 - stageWeight) + stageFan * stageWeight
        let mLen = length(mixed)
        finalDir = mLen > 0.001 ? mixed / mLen : stageFan
      } else {
        finalDir = blendedDir
      }
      ...
      for slot in 0..<3 {
        ...
        if styleChanged {
          applyStyle(targetStyle, to: &particles)
        }

        if particles.emitterShape != targetShape {
        ...
        particles.mainEmitter.angularSpeed = baseAngular * motionRate.multiplier
        particles.mainEmitter.angularSpeedVariation = particles.mainEmitter.angularSpeed * 0.75

        particles.mainEmitter.noiseStrength = baseStyleNoise + slotNoise
        particles.mainEmitter.noiseAnimationSpeed =
          motionRate.multiplier * (1.0 + (slot == 2 ? bop.z * 2.5 : 0.5))
